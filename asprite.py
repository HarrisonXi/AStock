...
def checkStockData(stockCode):
	...
	if len(transList) > 0:
		...
		# 排除买盘比卖盘还少的股票
		if testMode == False and buyVolume < sellVolume:
			return
		# 期间振幅计算
		swingRange = (maxPrice - minPrice) / transList[0].price * 100
		# 排除振幅十分小的股票
		if testMode == False and swingRange < 1:
			return
		# 获得股票名称等数据
		stock = requestStockData(stockCode)
		# 排除今日跌幅较大的股票
		if testMode == False and transList[-1].price < stock.yesterdayEnd * 0.97:
			return
		# 获取K线数据
		if timePercent < 1:
			# 获取前5天K线
			...
			# 有复牌情况则排除
			if testMode == False and klineList[0].date != correctDate:
				return
		...
		# 排除单日量比无爆发的股票
		if testMode == False and volumeRatio1 < 1.2:
			return
		# 计算K线得分
		klineScore = 0
		...
		if testMode == False and klineScore <= 12:
			return
		# 打印数据
		threadLock.acquire()
		print('==== ' + stockCode[2:] + ' ====')
...
countPerThread = (len(validStockList) + 99) / 100 if testMode == True else (len(validStockList) + 9) / 10
